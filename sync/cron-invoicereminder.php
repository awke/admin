...
$wantedemail->Body="$custname,\r\nAWKE Invoice number $rows[idinvoices] for the amount £$rows[total] was issued 2 weeks ago ($rows[invoicedate]) and according to our records has not been paid -(If you have made a payment within the last few days please ignore this) there is 2 weeks left before account associated with this will be disabled.  You can go to https://secure.awke.co.uk/admin/invoicedisplay.php?ID=$uniqid to view the invoice and see methods of payment.\n\nAWKE Accounts";
$wantedemail->Send();
}
...
$sql="SELECT * from invoices WHERE paid='0' AND void='0' AND ADDDATE(invoicedate,24)<=NOW() AND ADDDATE(invoicedate,25)>NOW()";
...
$mid="AWKE Invoice number $rows[idinvoices] for the amount of £$rows[total] was issued 4 weeks ago ($rows[invoicedate]) and according to our records has not been paid and is therefore overdue (If you have paid in the last few days please ignore this message).  We will be starting disabling procedures imminently, if you believe this to be incorrect please email or phone us ASAP.  You can go to https://secure.awke.co.uk/admin/invoicedisplay.php?ID=$uniqid to view the invoice and see methods of payment.\n\nAWKE Accounts";
...
$wantedemail->Body="$custname,\r\nAWKE Invoice number $rows[idinvoices] for the amount of £$rows[total] was issued 4 weeks ago ($rows[invoicedate]) and according to our records has not been paid and is therefore overdue (If you have paid in the last few days please ignore this message).  We will be starting disabling procedures imminently, if you believe this to be incorrect please email or phone us ASAP.  You can go to https://secure.awke.co.uk/admin/invoicedisplay.php?ID=$uniqid to view the invoice and see methods of payment.\n\nAWKE Accounts";
$wantedemail->Send();
}
 catch (phpmailerException $e) {
  echo $e->errorMessage(); //Pretty error messages from PHPMailer
